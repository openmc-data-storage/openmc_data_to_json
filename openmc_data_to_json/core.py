...
def trim_zeros_from_front_and_back_of_list(cross_section, energy):
    if len(cross_section) != len(energy):
        raise ValueError(
            "length of cross section array and energy array should be the same"
        )
    ...
    ofset2 = len(shorter_cross_section) - len(shorter_shorter_cross_section)

    if ofset2 == 0:
        shorter_shorter_energy = shorter_energy
    else:
        shorter_shorter_energy = shorter_energy[:-ofset2]

    return shorter_shorter_cross_section, shorter_shorter_energy
...
def cross_section_h5_to_json(
    ...
    for reaction in reaction_mt_numbers:
        ...
        for temperature in temperatures:
            ...
            ):  # this prevents natural (e.g. Carbon)

                neutron_number = int(
                    isotope_object._mass_number - isotope_object._atomic_number
                )
...
def reactions_in_h5(filename: str):
    isotope_object, particle = open_h5(filename)
    reactions = []
    for key, value in isotope_object.reactions.items():
